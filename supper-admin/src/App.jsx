...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${styles[status] || "bg-slate-50 text-slate-700 border-slate-200"}`}>
      {status}
    </span>
  );
};
    ...
    <div
      ...
      <div
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2.5 text-sm font-semibold text-white bg-red-600 rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-600/20">
            Confirm
          </button>
        </div>
      </div>
...
const ErrorState = ({ error, onRetry }) => (
  <div className="flex flex-col items-center justify-center py-16 px-4">
    ...
    {onRetry && (
      <button
        onClick={onRetry}
        className="px-6 py-2.5 text-sm font-semibold text-teal-700 bg-teal-50 rounded-xl hover:bg-teal-100 transition-colors">
        Retry
      </button>
    )}
  </div>
    ...
    <div
      ...
      <div className="hidden lg:flex lg:flex-[1.2] bg-gradient-to-br from-teal-600 to-emerald-600 flex-col justify-center p-16 xl:p-24 text-white relative overflow-hidden">
        ...
        <p className="text-xl opacity-90 max-w-md leading-relaxed relative">
          Monitor, manage, and scale your pharmacy SaaS platform from a single
          unified dashboard.
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <button
              ...
              {loading ? (
                ...
                <>
                  Sign In <ArrowUpRight className="w-5 h-5" />
                </>
              )}
            </button>
          </form>
...
const SubscriptionsManager = ({ isAuthenticated, queryClient }) => {
  ...
  const updateTierLimit = (tierId, field, value) => {
    setLocalTiers((prev) => ({
      ...
      [tierId]: {
        ...prev[tierId],
        limits: { ...prev[tierId].limits, [field]: Number(value) },
      },
    }));
  };
  ...
  const updateTierPricing = (tierId, field, value) => {
    setLocalTiers((prev) => ({
      ...
      [tierId]: {
        ...prev[tierId],
        pricing: { ...prev[tierId].pricing, [field]: Number(value) },
      },
    }));
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage plan pricing, limits, and features
          </p>
        </div>
          ...
          {editMode ? (
            <>
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 text-sm font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-700 transition-all shadow-lg shadow-teal-600/20 active:scale-[0.98] disabled:opacity-70">
            ...
            <button
              ...
              className="px-6 py-2.5 text-sm font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-700 transition-all shadow-lg shadow-teal-600/20 active:scale-[0.98]">
              Edit Tiers
            </button>
          )}
        </div>
          ...
          <div
            ...
            <div className="flex items-center justify-between mb-4">
              {editMode ? (
                <input
                  ...
                  className="text-lg font-bold text-slate-900 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1 w-full"
                />
              ) : (
              ...
              {editMode && (
                <button
                  onClick={() => toggleTierEnabled(tierId)}
                  className={`ml-2 px-3 py-1 text-xs font-semibold rounded-lg ${tier.enabled ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
            ...
            {editMode ? (
              <textarea
                ...
                onChange={(e) =>
                  updateTier(tierId, "description", e.target.value)
                }
                className="text-sm text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 w-full mb-4 resize-none"
                rows={2}
              />
            ) : (
                ...
                {editMode ? (
                  <>
                    <input
                      ...
                      className="text-2xl font-black text-slate-900 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 w-24"
                    />
                    <span className="text-sm text-slate-500">/mo</span>
                    <input
                      type="number"
                      value={tier.pricing.yearly}
                      onChange={(e) =>
                        updateTierPricing(
                          tierId,
                          "yearly",
                      ...
                      className="text-lg font-bold text-slate-700 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 w-24 ml-2"
                    />{" "}
                  ...
                  <>
                    <span className="text-2xl font-black text-slate-900">
                      ETB {tier.pricing.monthly.toLocaleString()}
                    </span>
                    <span className="text-sm text-slate-500">/mo</span>
                  </>
                )}
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    {editMode ? (
                      <input
                        type="number"
                        value={tier.limits[key]}
                        onChange={(e) =>
                          updateTierLimit(tierId, key, Number(e.target.value))
                        }
                        className="w-16 bg-slate-50 border border-slate-200 rounded px-2 py-0.5 text-xs"
                      />
                    ) : (
                      <span className="font-semibold">
                        {tier.limits[key] === -1 ? "∞" : tier.limits[key]}
                      </span>
                    )}
                  </div>
                ))}
            ...
            <div className="border-t border-slate-100 pt-4 mt-4">
              ...
              {editMode ? (
                <textarea
                  ...
                  className="text-sm text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 w-full resize-none"
                  rows={5}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-600/20 focus:border-teal-600 transition-all"
            />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">
          ...
          <div>
            ...
            <button
              ...
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${localSettings.maintenanceMode ? "translate-x-6" : "translate-x-1"}`}
              />
            </button>
          </div>
        </div>
...
export default function App() {
  ...
  const pharmaciesQuery = useQuery({
    queryKey: ["adminPharmacies", pharmacyFilter],
    queryFn: () => fetchPharmacies({ status: pharmacyFilter }),
    enabled: isAuthenticated,
  });
  const pharmacyDetailQuery = useQuery({
    queryKey: ["adminPharmacyDetail", selectedPharmacyId],
    queryFn: () => fetchPharmacyDetail(selectedPharmacyId),
    enabled: isAuthenticated && !!selectedPharmacyId,
  });
  ...
  const paymentsQuery = useQuery({
    queryKey: ["adminPayments", paymentFilter],
    queryFn: () => fetchPayments({ status: paymentFilter }),
    enabled: isAuthenticated,
  });
  ...
  const statusMutation = useMutation({
    mutationFn: ({ pharmacyId, status, reason }) =>
      updatePharmacyStatus(pharmacyId, status, reason),
    onSuccess: () => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "MRR",
              value: `ETB ${(stats.mrr || 0).toLocaleString()}`,
              icon: DollarSign,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                  data={Object.entries(stats.planDistribution || {}).map(
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Manage all registered pharmacies on the platform
            </p>
          </div>
          <select
                ...
                {pharmacies.length === 0 ? (
                  ...
                  pharmacies.map((p) => (
                    <tr
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        {p.usageMetrics?.currentBranchCount || 1}
                      </td>
                      <td className="px-6 py-4 text-slate-600">
                        {p.usageMetrics?.currentUserCount || 1}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => {
                              ...
                              handlePageChange("pharmacy-detail");
                            }}
  ...
  const renderPharmacyDetail = () => {
    ...
    if (pharmacyDetailQuery.isLoading) return <LoadingState />;
    if (pharmacyDetailQuery.error)
      return (
        <ErrorState
          error={pharmacyDetailQuery.error}
          onRetry={() => pharmacyDetailQuery.refetch()}
        />
      );
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <button
            onClick={() => handlePageChange("pharmacies")}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-sm font-semibold text-slate-900 mt-1 capitalize">
                    {pharmacy.subscription?.tier || "starter"}
                  </div>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
                  ...
                  <div className="text-sm font-semibold text-slate-900 mt-1 capitalize">
                    {pharmacy.subscription?.billingCycle || "monthly"}
                  </div>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900 mb-4">
                Members ({pharmacy.members?.length || 0})
              </h3>
              ...
              <div className="space-y-2">
                {pharmacy.status !== "active" && (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-700 transition-all shadow-lg shadow-teal-600/20 active:scale-[0.98] disabled:opacity-70">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              ...
              <div className="space-y-4">
                {[
                  {
                    label: "SKUs",
                    current: pharmacy.usageMetrics?.currentSkuCount || 0,
                    max: 500,
                    color: "bg-teal-600",
                  },
                  {
                    label: "Users",
                    current: pharmacy.usageMetrics?.currentUserCount || 0,
                    max: 5,
                    color: "bg-blue-600",
                  },
                  {
                    label: "Branches",
                    current: pharmacy.usageMetrics?.currentBranchCount || 0,
                    max: 2,
                    color: "bg-amber-600",
                  },
                ].map((m, i) => (
                  <div key={i}>
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div
                        className={`${m.color} h-2 rounded-full transition-all duration-500`}
                        style={{
                          width: `${Math.min(100, (m.current / m.max) * 100)}%`,
                        }}></div>
                    </div>
                  </div>
                ))}
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Review and approve pending pharmacy registrations
          </p>
        </div>
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["Pharmacy", "Type", "Location", "Submitted", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
                        className="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-slate-500">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Transaction history across all pharmacies
            </p>
          </div>
          <select
            value={paymentFilter}
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            SaaS business metrics and insights
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "Active Rate",
              value: `${stats.totalPharmacies ? Math.round(((stats.activePharmacies || 0) / stats.totalPharmacies) * 100) : 0}%`,
            },
          ].map((s, i) => (
            <div
              key={i}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["Actor", "Action", "Target", "Details", "Timestamp"].map(
                    (h) => (
                      <th
                        key={h}
                        className="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-slate-500">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Send platform-wide messages to pharmacies
            </p>
          </div>
          <button
            onClick={() =>
            ...
            <div
              ...
              <div className="flex items-center gap-4 text-xs text-slate-400 font-medium mt-4 pt-4 border-t border-slate-100">
                <span className="flex items-center gap-1.5">
                  <Target className="w-3.5 h-3.5" /> {a.target || "All"}
                </span>
                <span className="flex items-center gap-1.5">
    ...
    if (featureFlagsQuery.error)
      return (
        <ErrorState
          ...
          onRetry={() => featureFlagsQuery.refetch()}
        />
      );
  ...
  return (
    <div
      ...
      <aside
        className={`fixed lg:sticky top-0 left-0 h-screen bg-white border-r border-slate-200 z-50 transition-all duration-200 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} ${sidebarOpen ? "w-64" : "w-20"}`}>
        <div className="flex flex-col h-full">
          <div
            className={`flex items-center h-16 border-b border-slate-100 ${sidebarOpen ? "px-6" : "px-4 justify-center"}`}>
            ...
            <ul className="space-y-0.5">
              {navItems.map((item) => {
                ...
                const isActive =
                  activePage === item.id ||
                  (item.id === "pharmacies" &&
                    activePage === "pharmacy-detail");
                return (
                  <li key={item.id}>
                    <button
                      onClick={() => handlePageChange(item.id)}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? "bg-teal-50 text-teal-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"} ${!sidebarOpen ? "justify-center" : ""}`}
                      title={!sidebarOpen ? item.label : ""}>
                      <Icon
                        className={`w-5 h-5 flex-shrink-0 ${isActive ? "text-teal-600" : "text-slate-400"}`}
                      />
          ...
          <div
            className={`border-t border-slate-100 p-3 ${!sidebarOpen ? "flex justify-center" : ""}`}>
            <button
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to proceed? This action may affect platform operations."
      />
    </div>
  );
}
