...
export const completeRegistration = async (req, res) => {
  try {
    ...
    const pharmacyId = await db.runTransaction(async (transaction) => {
      ...
      if (!userDoc.exists) throw new Error("User profile not found. Please restart signup.");

      // Calculate subscription end date
      const periodEnd = new Date();
      periodEnd.setDate(periodEnd.getDate() + (subscriptionData.billingCycle === 'yearly' ? 365 : 30));
