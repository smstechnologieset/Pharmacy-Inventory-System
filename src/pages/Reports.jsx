...
const Reports = () => {
  ...
  const [batches, setBatches] = useState([]); // Added for split data model
  ...
  const inventoryCategoryChartData = useMemo(() => {
    const totals = enrichedBatches.reduce((acc, b) => {
      const cat = b.category || "Uncategorized";
      acc[cat] = (acc[cat] || 0) + Number(b.stock || 0);
      return acc;
    }, {});
  ...
  const handleExport = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`PharmaCare ${activeTab} Report`, 14, 16);
    doc.setFontSize(12);
    doc.text(`Period: ${reportPeriod}`, 14, 24);
    ...
    if (activeTab === "Sales") {
      ...
      y += 8;
      doc.text(`Real Profit: ETB ${totalRealProfit.toLocaleString()}`, 14, y);
    } else if (activeTab === "Inventory") {
      doc.text(`Total Stock: ${totalStock} units`, 14, y);
      y += 8;
      doc.text(`Low Stock Batches: ${lowStockCount}`, 14, y);
    } else if (activeTab === "Profit") {
      doc.text(`Real Profit: ETB ${totalRealProfit.toLocaleString()}`, 14, y);
    } else if (activeTab === "Expiration") {
      doc.text(`Expired Batches: ${expiredBatches.length}`, 14, y);
      y += 8;
      doc.text(`Expiring Soon: ${expiringSoonBatches.length}`, 14, y);
    }
    doc.save(
      `pharmastock-${activeTab.toLowerCase()}-${reportPeriod.toLowerCase()}.pdf`,
    );
